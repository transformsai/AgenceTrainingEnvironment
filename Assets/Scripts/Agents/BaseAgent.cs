...
public abstract class BaseAgent : Agent
{
    ...
    public BaseAgent pushTarget = null;
    public TrainingConsumePoint priorConsumePoint = null;
    public TensorboardObs tensorboardObs;
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        if(lightning!= null)
            LogVectorObs("lightning target position", lightning.transform.position, new Vector3(-0.5f, 0, -0.5f), new Vector3(0.5f, 0.6f, 0.5f), sensor);
        else
        ...
        LogVectorObs("Consuming", IsConsuming, sensor);
        ...
        foreach (var agent in agents)
        {
            if (Vector3.Distance(agent.transform.position, this.transform.position) < 2f)
                pushTarget = agent.GetComponent<BaseAgent>();
    ...
    public void SetLightningPos(GameObject lightning)
    {
        this.lightning = lightning;
    }

    public void Consume(TrainingConsumePoint point)
    {
        IsConsuming = true;
        priorConsumePoint = point;
        consumeCount++;
    }
    ...
    public float GetPushActionTime()
    {
        return pushActionTime;
    }

    public void SetPushActionTime(float newTime)
    {
        pushActionTime = newTime;
    }

    public Rigidbody GetPlanet()
    {
        return planet;
    }
    ...
    public void Reset()
    {
        ...
        tensorboardObs.UpdateStatsRecorder(this.name + " Consume", consumeCount);
        tensorboardObs.UpdateStatsRecorder(this.name + " Push", pushCount);
    ...
    public override void OnActionReceived(float[] vectorAction)
    {
        ...
        if (push > 0 && pushTarget && !pushTarget.IsStunned && Time.fixedTime > pushTime && Time.fixedTime > pushActionTime)
        {
            pushActionTime = Time.fixedTime + 2;
            pushTarget.Push();
        }
        ...
        if (!IsConsuming && Time.fixedTime > pushTime && Time.fixedTime > pushActionTime)
        {
            IsStunned = false;
            IsPushing = false;
        ...
        animator.SetBool("IsConsuming", IsConsuming);
    }
